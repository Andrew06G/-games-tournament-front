...
export default function EquiposHub() {
  ...
  return (
    <div className="flex min-h-screen flex-col bg-[#f9f9f9] font-sans text-[#1b1b1b]">
      <ArenaHeader active="equipos" bg="gray" />
      <main className="mx-auto w-full max-w-5xl flex-1 px-6 py-10 md:px-10">
        <h1 className="text-3xl font-bold text-black">Equipos por torneo</h1>
        <p className="mt-2 text-sm text-[#5c5f60]">
          Elija un torneo para ver o inscribir equipos.
        </p>
          ...
          {(data ?? []).map((t) => (
            <li key={t.idTorneo}>
              ...
              >
                ...
                <span className="text-sm text-[#5c5f60]">Gestionar equipos →</span>
              </Link>
            </li>
          ))}
        </ul>
